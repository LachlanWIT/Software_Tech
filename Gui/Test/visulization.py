...
cleanlinessKeywords = ['clean', 'neat', 'fresh', 'hygienic', 'taintless', 'sterile', 'sanitary', 'washed', 'flawless',
...
def findKeyword(startperiod, endperiod, keyword):
    ...
    listingsReducedColumns['matchedAmenities'] = listingsReducedColumns.amenities.apply(
        lambda row: 'match' if keyword in row else 'no match')
...
def getListings(startperiod, endperiod, suburbName='sydney', keyword=None):
    ...
    if keyword != None:
        ...
        if len(filteredListings[keyword]):
            ...
            listingsReducedColumns['matchedSuburb'] = listingsReducedColumns.id.apply(
                lambda row: 'match' if int(row) in filteredListings[keyword] else 'no match')
